...
client = MongoClient("mongodb+srv://test:test@cluster0.kidfw8e.mongodb.net/")

chat_db = client.get_database("ChatDB") # Name of databse on mongoDB platform with Collection (analogous to RDBMS) called "users"
...
def save_user(username, email, password):
    ...
    # if existing_user:
    # users_collection.update_one({'_id': username},{'$set': {'email': email, 'password': password_hash}})
    #else:
    users_collection.insert_one({'_id' : username, 'email' : email, 'password' : password_hash}) # username will serve as the primary key 


#save_user("ahmed", "example@b.com","test1")
